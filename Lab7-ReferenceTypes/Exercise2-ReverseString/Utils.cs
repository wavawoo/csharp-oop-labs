namespace Utils
{
    ...
    class Utils
    {
        ...
        // Метод для обращения строки (передача по ссылке)
        public static void Reverse(ref string s)
        {
            string sRev = "";  // Новая строка для результата

            // Проход по исходной строке с конца к началу
            for (int i = s.Length - 1; i >= 0; i--)
            {
                sRev += s[i];  // Добавление символов в обратном порядке
            }

            s = sRev;  // Замена исходной строки на обращенную
        }
    }
}
